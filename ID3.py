...
Version: v1
Functionality: Prints the steps taken by ID3 algorithm while making the tree
How To Run: Call the ID3 function and as an arguement give it the location of the CSV file as a string
Assumptions: Assumes the first line of the csv file contains column names and that the target variable is the last in each row
...
def calculate_information(total_instances: int, occurences: int) -> float:
    ...
    """
    # Assertions
    assert type(total_instances) == int and type(
        occurences) == int, "Parameters must be ints"
...
def calculate_entropy(data: List[Tuple]) -> float:
    ...
    assert all([type(item) == tuple for item in data]
               ), "data should be a list of points"
    ...
    if (len(c.items()) == 1):
        ...
        informations = [calculate_information(
            total_instances, propotion[-1]) for propotion in c.items()]
...
def calculate_information_gain(data: List[Tuple], attribute: str, attribute_list: List[str]) -> float:
    ...
    """
    # Assertions
    assert type(data) == list and type(
        attribute_list) == list, "data and attribute_list must be lists"
    ...
    assert all([len(attribute_list) == len(item) for item in data]
               ), "All attributes in attribute list should be in data"
...
def get_information_gains(data: List[Tuple], attribute_list: List[str]) -> Dict[str, float]:
    ...
    # Assertions
    assert type(data) == list and type(
        attribute_list) == list, "data and attribute list should be lists"
    assert all([type(item) == tuple for item in data]
               ), "data should contain points"
    assert all([len(attribute_list) == len(item) for item in data]
               ), "data should contain as many value as attribute list"

    # Get information gain of every attribute
    information_gains = defaultdict(float)
    for i in range(len(attribute_list) - 1):
        information_gains[attribute_list[i]] = round(
            calculate_information_gain(data, attribute_list[i], attribute_list), 4)
...
def select_best_attribute(information_gains: Dict[str, float]) -> str:
    ...
    # Assertions
    assert type(
        information_gains) == dict, "information gains should be a dictionary"
...
def split_data(data: List[Tuple], splitting_attribute: str, attribute_list: List[str]) -> Dict[str, List]:
    ...
    # Assertions
    assert type(data) == list and type(
        attribute_list) == list, "data and attribute_list are lists"
    assert type(
        splitting_attribute) == str, "splitting attribute should be a string"
    assert splitting_attribute in attribute_list, "splitting attribue should be in attribute list"
    assert all([type(item) == tuple for item in data]
               ), "data should contain a list of points"
    assert all(len(attribute_list) == len(item)
               for item in data), "data points should have as many attributes as attribute_list"
...
def create_tree(data: List[Tuple], attribute_list: List[str], node: Type[Node] = None):
    ...
    print(f"\n\ndata is {data}")
    print(
        f"Parent Attribute is: {node.parent_attr_type}, {node.parent_attribute}")
    ...
    splitted_data = split_data(
        data=data, splitting_attribute=splitting_node, attribute_list=attribute_list)
    ...
    node.children = [Node(parent_attribute=key, parent_attr_type=splitting_node)
                     for key in new_split_data.keys()]
    print(f"\n\nFUNCTION END\n\n")
...
def ID3(csv_file: str) -> None:
    """
    Given a file location csv_file it prints how the ID3 algorithm constructs the decision tree
    """
    # Assertions
    assert type(csv_file) == str, "csv_file should be a string"
    assert exists(csv_file), "File Does Not Exist"
